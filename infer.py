...
parser.add_argument('--infer_input_file', type=str, default='')
...
if not isinstance(args.infer_input_file, list):
    args.infer_input_file = [args.infer_input_file]
        ...
        if 'other_res' in batch:
            ...
            if add_acc:
                ...
                for k, v in batch_other_res['acc_map'].items():
                    if k not in batch_other_res or v not in encoded_info:
                        continue
    ...
    if not args.only_generate:
        if 'acc' in other_res:
            for k, v in other_res['acc_map'].items():
                if k not in other_res['acc'] or v not in other_res['acc']:
                    continue
