...
# basic utils
# `ICL` stores retrieved experiential exemplars; the legacy `comet` name stores CB-CoT output.
class InputFeatures(object):
    def __init__(
            self,
            input_ids, comet_inputs_ids,
            decoder_input_ids, labels, persona_input_ids, ICL_input_ids
    ):
        ...
        self.ICL_input_ids = ICL_input_ids
        self.ICL_input_length = len(ICL_input_ids)

        self.input_len = self.input_length + self.decoder_input_length

        self.padding_length = max(self.input_length, self.persona_input_length, self.ICL_input_length)
        self.comet_inputs_ids = comet_inputs_ids[:self.padding_length]


def featurize(
        bos, eos, ICL, persona, comet,
    ...
    if concat_ctx_persona:
        ...
        if not encode_context:
            ...
            context = '</s> <s>'.join(context)
            context = toker(context)
            ICL_input_ids = toker(ICL).input_ids[-max_input_length:]
            ...
            comet_inputs_ids = toker(comet).input_ids[-max_input_length:]
            ...
            if not use_all_persona:
                if use_prompt:
                    ...
                    if user_number > 2:
                        ...
                        cot_node = ['persona_situation', 'persona_thought', 'persona_emotion', 'persona_behavior']
                        cot_node_rm = ["persona's situation", "persona's thought", "persona's emotion", "persona's behavior"]
                        com = "<persona> "
                        for index, node in enumerate(cot_node):
                            c = []
                            item = dialog[i][node]

                            if index != 2:
                                com = com + cot_node_rm[index] + ': ' + item + ' '
                            else:
                                if len(item) > 0:
                                    item = ' '.join(item)
                                    item = item.translate(str.maketrans('', '', punctuation))
                                    c.append(item)
                                com = com + cot_node_rm[index] + ': ' + ', '.join(c) + '. '
                        com += "<input>"
                        ...
                        if len(dialog[i]) > 3:
                            ICL = ""
                            first_dpr = dialog[i]['dpr'][0]
                            if last_text == first_dpr[0] and text == first_dpr[2]:
                                for dpr in dialog[i]['dpr'][1:4]:
...
def convert_inputs_to_features(inputs, toker, encode_context, **kwargs):
    ...
    for i in range(len(inputs)):
        ipt = inputs[i]
        feat = featurize(
            bos, eos, ipt['ICL'], ipt['persona'], ipt['comet'],
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        ICL_input_ids = my_pad_sequence([torch.tensor(f.ICL_input_ids, dtype=torch.long) for f in features],
                                            batch_first=True, max_len=max_len, padding_value=pad)
        ICL_attention_mask = my_pad_sequence(
            [torch.tensor([1.] * f.ICL_input_length, dtype=torch.float) for f in features],
            batch_first=True, max_len=max_len, padding_value=0.)
        ...
        comet_input_ids = my_pad_sequence([torch.tensor(f.comet_inputs_ids, dtype=torch.long) for f in features],
                                          batch_first=True, max_len=max_len, padding_value=pad)
        comet_attention_mask = my_pad_sequence(
            [torch.tensor([1.] * len(f.comet_inputs_ids), dtype=torch.float) for f in features],
            batch_first=True, max_len=max_len, padding_value=0.)
        if not infer:
        ...
        res = {
            ...
            "ICL_attention_mask": ICL_attention_mask,
        }
