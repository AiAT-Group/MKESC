# Adapted from the Hugging Face BART implementation.
...
class Model(BaseModel, BlenderbotSmallForConditionalGeneration):
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        self.ICL_norm = nn.LayerNorm(self.model.config.d_model, elementwise_affine=True)  # 512
        ...
        self.context_norm_3 = nn.LayerNorm(self.model.config.d_model, elementwise_affine=True)  # 512
        ...
        self.persona_context_w = nn.Parameter(torch.tensor([1 / 7, 1 / 7, 1 / 7, 1 / 7, 1 / 7, 1 / 7, 1 / 7]))
        self.my_past = None
        self.my_encoder_outputs = None
        self.generation_strategy = None
        self.train_alpha = False
        self.strategy_alpha = [0, 0, 0, 0, 0, 0, 0, 0]
        # Strategy tokens are defined in CONFIG/strat.json.
    ...
    # `comet_*` is a legacy name for the CB-CoT cognitive-affective knowledge stream.
    def forward(
            self,
            input_ids=None,
            attention_mask=None,
            ICL_input_ids=None,
            ICL_attention_mask=None,
        ...
        if not persona_change_attention or persona_input_ids is None:
            ...
            # ICL
            ICL_encoder_outputs = self.model.encoder(
            ...
            context = torch.stack([torch.matmul(torch.softmax(torch.matmul(j, i.t()), dim=-1), i) for i, j in
                                   zip(encoder_outputs[0], ICL_encoder_outputs[0])])
            ICL = torch.stack([torch.matmul(torch.softmax(torch.matmul(i, j.t()), dim=-1), j) for i, j in
                                   zip(encoder_outputs[0], ICL_encoder_outputs[0])])
            context_ICL = self.ICL_norm(context+ICL_encoder_outputs.last_hidden_state)
            ICL = self.context_norm_1(encoder_outputs.last_hidden_state+ICL)
            ...
            persona = self.context_norm_2(encoder_outputs.last_hidden_state+persona)
            ...
            comet = self.context_norm_3(encoder_outputs.last_hidden_state + comet)
            ...
            w6 = torch.exp(self.persona_context_w[5]) / torch.sum(torch.exp(self.persona_context_w))
            w7 = torch.exp(self.persona_context_w[6]) / torch.sum(torch.exp(self.persona_context_w))
            encoder_outputs.last_hidden_state = w1 * encoder_outputs.last_hidden_state + w2 * context_ICL + w3 * ICL + w4 * context_persona + w5 * persona + w6 * context_comet + w7 * comet
            encoder_outputs.last_hidden_state = self.norm(w1 * encoder_outputs.last_hidden_state + w2 * context_ICL + w3 * ICL + w4 * context_persona + w5 * persona + w6 * context_comet + w7 * comet)
